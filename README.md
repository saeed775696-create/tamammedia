...
- تأكد أن `analyticsdata.googleapis.com` مفعّلة في مشروع `project_id` الموجود داخل ملف JSON.
- أضف `client_email` من ملف JSON في **Google Analytics ← المشرف ← إدارة وصول الخاصية** بصلاحية Viewer، وليس كدور IAM في Google Cloud فقط.
- استخدم Property ID الرقمي من إعدادات خاصية GA4، ولا تستخدم Measurement ID الذي يبدأ بـ `G-` مكانه.
- أي مفتاح خاص نُشر أو أُرسل خارج القناة المخصصة يُعد مكشوفًا ويجب حذفه من Google Cloud وإنشاء مفتاح جديد.
