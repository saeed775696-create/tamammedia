      ...
      renderCard={(item, { onEdit, onDelete }) => (
        <Card key={item.id} className="group overflow-hidden hover:-translate-y-0.5 transition-all p-0">
          <div className="relative h-40 sm:h-48 bg-surface-100 overflow-hidden rounded-t-xl">
            ...
            <span className="absolute top-2 start-2 bg-white/90 backdrop-blur-md text-caption px-2 py-0.5 rounded font-bold text-brand-900 shadow-sm">
          ...
          <div className="p-3 sm:p-4 relative">
            <h3 className="text-h6 font-bold text-brand-900 line-clamp-1 mb-0.5">{item.titleAr}</h3>
            <Text variant="small" className="line-clamp-1">{item.titleEn}</Text>
            {item.clientName && (
              <div className="mt-2 pt-2 border-t border-surface-100 flex items-center gap-1.5">
                <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-accent-100 text-accent-600">
                  <Users size={10} strokeWidth={3} />
                </div>
                <span className="text-caption font-bold text-surface-600 truncate">{item.clientName}</span>
              </div>
            )}
          </div>
        </Card>
      ...
      renderModal={(crud) => (
        <Modal isOpen={crud.isAddModalOpen} onClose={() => crud.setIsAddModalOpen(false)} title={crud.editItem ? "تعديل بيانات العمل" : "إضافة عمل جديد للمعرض"}>
          <div className="space-y-4 mt-3">
            <div className="bg-gradient-to-r from-brand-50 to-surface-50 p-4 rounded-xl border border-brand-100/50 flex items-start gap-3 shadow-sm">
              <div className="bg-white p-2 rounded-lg shadow-sm"><FolderOpen className="text-brand-500" size={20} strokeWidth={1.5} /></div>
              <div>
                <h4 className="text-label font-bold text-brand-900">معلومات العمل الأساسية</h4>
                <Text variant="caption" className="mt-1 leading-normal">أدخل تفاصيل المشروع بدقة لضمان ظهوره بشكل احترافي في معرض الأعمال.</Text>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-4 bg-white p-4 rounded-xl border border-surface-100 shadow-sm">
                <h5 className="text-caption font-extrabold text-surface-400 uppercase tracking-wider mb-3 border-b border-surface-100 pb-1.5">التفاصيل النصية</h5>
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="flex items-center gap-1.5 text-label font-bold text-surface-700"><Edit2 size={13} className="text-accent-500" /> العنوان العربي <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-1">
                    <label className="flex items-center gap-1.5 text-label font-bold text-surface-700"><Edit2 size={13} className="text-accent-500" /> العنوان الإنجليزي <span className="text-red-500">*</span></label>
              ...
              <div className="space-y-4 bg-white p-4 rounded-xl border border-surface-100 shadow-sm">
                <h5 className="text-caption font-extrabold text-surface-400 uppercase tracking-wider mb-3 border-b border-surface-100 pb-1.5">التصنيف والعميل</h5>
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="flex items-center gap-1.5 text-label font-bold text-surface-700"><Filter size={13} className="text-accent-500" /> القسم <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-1">
                    <label className="flex items-center gap-1.5 text-label font-bold text-surface-700"><Users size={13} className="text-accent-500" /> اسم العميل <span className="text-surface-400 font-normal">(اختياري)</span></label>
            ...
            <div className="bg-white p-4 rounded-xl border border-surface-100 shadow-sm space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-start">
                <div className="flex-1 w-full space-y-2">
                  <Button variant="outline" className="relative overflow-hidden cursor-pointer w-full justify-center border-dashed border hover:border-brand-300 hover:bg-brand-50" leftIcon={<ImageIcon size={16} className="text-brand-500" />} isLoading={isUploading}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-caption text-surface-400 font-bold uppercase">أو</span>
                    <div className="h-px bg-surface-200 flex-1"></div>
                  </div>
                  <Input name="imageUrl" value={form.imageUrl} onChange={(e) => setForm({...form, imageUrl: e.target.value})} placeholder="ألصق رابط الصورة مباشرة..." dir="ltr" className="bg-surface-50 focus:bg-white text-body-sm" />
                </div>
                {form.imageUrl ? (
                  <div className="w-full sm:w-28 h-28 rounded-lg border border-surface-200 overflow-hidden relative group bg-surface-50 shrink-0">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <div className="w-full sm:w-28 h-28 rounded-lg border border-dashed border-surface-300 bg-surface-50 flex items-center justify-center shrink-0">
                    <ImageIcon size={20} className="text-surface-300" />
                  </div>
                )}
              </div>
            </div>
            <label className="flex items-start gap-3 cursor-pointer p-4 bg-gradient-to-r from-amber-50 to-white border border-amber-200/60 rounded-xl hover:border-amber-300 hover:shadow-sm transition-all group">
              <div className="pt-0.5 relative">
                ...
                <div className="w-4.5 h-4.5 border-2 border-surface-300 rounded peer-checked:bg-amber-500 peer-checked:border-amber-500 flex items-center justify-center transition-colors">
                  <Star size={11} className="text-white opacity-0 peer-checked:opacity-100 transition-opacity" fill="currentColor" />
                </div>
              </div>
              <div className="flex-1">
                <span className="text-label font-bold text-amber-900 flex items-center gap-1.5 group-hover:text-amber-700 transition-colors"><Star size={16} className="text-amber-500" fill="currentColor" /> تمييز هذا المشروع في الصفحة الرئيسية</span>
                <Text variant="caption" className="mt-0.5 text-amber-700/80">المشاريع المميزة تحصل على نسبة ظهور أعلى وتلفت انتباه زوار الموقع بشكل أكبر.</Text>
              </div>
            </label>
